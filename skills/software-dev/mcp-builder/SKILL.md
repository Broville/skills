...
**Recommended stack:**
- **Language**: TypeScript (high-quality SDK support and good compatibility in many execution environments e.g. MCP-compatible client. Plus AI models are good at generating TypeScript code, benefiting from its broad usage, static typing and good linting tools)
...
- Designing tools with vague names that agents cannot discover.
- Returning huge unfiltered payloads.
- Skipping read-only evaluation questions before release.

## Verification

- [ ] `npm run build` or `python -m py_compile` succeeds.
- [ ] MCP Inspector runs and lists tools with clear descriptions.
- [ ] At least one read-only evaluation question has a verifiable answer.
