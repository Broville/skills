...
3. **Fetch the smallest suitable resource.** Use a chapter endpoint for reading, citation, or passage study. Use `complete.json` only for an offline pack, indexing job, or bulk analysis; it can be several megabytes. For bulk work, use bounded timeouts, limited retries with backoff, and caching only where source terms permit it. Completion: the response parses as JSON and its returned metadata matches the selected resource.
