#!/usr/bin/env python3
"""
Inspect failing GitHub PR checks, fetch GitHub Actions logs, and extract
failure snippets. Exits non-zero when failures remain so it can be used
in automation.

Requires:
  - `gh` authenticated (run `gh auth login` once)
  - Python 3.8+ (uses only stdlib)

Usage:
  python3 inspect_pr_checks.py --repo . --pr 123
  python3 inspect_pr_checks.py --repo . --pr https://github.com/org/repo/pull/123 --json
  python3 inspect_pr_checks.py --repo . --max-lines 200 --context 40
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect failing GitHub PR checks, fetch GitHub Actions logs, "
